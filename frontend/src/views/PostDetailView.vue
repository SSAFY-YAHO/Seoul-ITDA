  ...
  <div class="page-shell">
    <section class="section-card section-block">
      ...
      <div v-else-if="error" class="error-state">
        ...
        <p>{{ error }}</p>
        <button class="btn btn--secondary" type="button" @click="loadPost">다시 시도</button>
      </div>
      ...
      <div v-else>
        <div class="section-heading">
          ...
          <div class="inline-actions">
            <button class="btn btn--secondary" type="button" @click="router.push('/posts')">목록</button>
            <button class="btn btn--ghost" type="button" @click="router.push(`/posts/${post.id}/edit`)">수정</button>
          </div>
        </div>
        ...
        <div class="detail-card detail-card--compact">
          <h3>삭제하기</h3>
          <p class="helper-text">실제 비밀번호는 저장되지 않으며, 백엔드 스펙에 따라 검증됩니다. 개인정보를 포함하지 않은 비밀번호를 입력하세요.</p>
          <label class="field" for="delete-password">
            <span>수정용 비밀번호</span>
            <input id="delete-password" v-model="password" class="input" type="password" placeholder="비밀번호를 입력하세요" />
          </label>
          <div class="inline-actions">
            <button class="btn btn--primary" type="button" :disabled="deleting" @click="handleDelete">{{ deleting ? '삭제 중...' : '삭제' }}</button>
          </div>
