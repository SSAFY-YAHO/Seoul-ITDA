...
async function submitEdit() {
  ...
  try {
    await updatePost(postId.value, { title: title.value, content: content.value, password: password.value })
    ...
    <section class="section-card section-block">
      ...
      <div v-else>
        <div class="section-heading">
          ...
          <button class="btn btn--ghost" type="button" @click="router.push(`/posts/${postId}`)">상세 보기</button>
        </div>

        <div class="form-grid">
          <label class="field" for="edit-title">
            <span>제목</span>
            <input id="edit-title" v-model="title" class="input" placeholder="제목을 입력하세요" />
          </label>
          <label class="field" for="edit-content">
            <span>내용</span>
            <textarea id="edit-content" v-model="content" class="textarea" placeholder="내용을 입력하세요" />
          </label>
          <label class="field" for="edit-password">
            <span>수정용 비밀번호</span>
            <input id="edit-password" v-model="password" class="input" type="password" placeholder="비밀번호를 입력하세요" />
          </label>
          <p v-if="error" class="form-error">{{ error }}</p>
          <button class="btn btn--primary" type="button" :disabled="submitting" @click="submitEdit">{{ submitting ? '수정 중...' : '수정하기' }}</button>
        </div>
      </div>
    </section>
  </div>
