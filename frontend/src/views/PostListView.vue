    ...
    <section class="section-card section-block">
      <div class="section-heading">
        <div>
          ...
          <h2>익명 게시글</h2>
          <p class="page-subtitle">로그인 없이 제목과 내용으로 소중한 경험을 나눠보세요.</p>
        </div>
        <button class="btn btn--primary" type="button" @click="router.push('/posts/new')">글쓰기</button>
      </div>

      <div class="search-row">
        <label class="sr-only" for="post-search">게시글 검색</label>
        <input id="post-search" v-model="searchInput" class="input" placeholder="제목이나 내용을 검색해보세요" @keyup.enter="submitSearch" />
        <button class="btn btn--secondary" type="button" @click="submitSearch">검색</button>
        <button v-if="searchQuery" class="btn btn--ghost" type="button" @click="clearSearch">초기화</button>
      </div>
      ...
      <div v-else-if="error" class="error-state">
        ...
        <p>{{ error }}</p>
        <button class="btn btn--secondary" type="button" @click="loadPosts">다시 시도</button>
      </div>
      ...
      <div v-else class="card-list card-list--stacked">
        <article v-for="post in posts" :key="post.id" class="post-card" @click="router.push(`/posts/${post.id}`)">
          ...
          <div class="post-card__footer">
            ...
            <button class="btn btn--ghost btn--small" type="button" @click.stop="router.push(`/posts/${post.id}`)">자세히 보기</button>
          </div>
        </article>
      </div>
    </section>
